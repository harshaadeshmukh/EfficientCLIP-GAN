...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='CLIP-GAN',
                        help='the model for training')
    parser.add_argument('--state_epoch', type=int, default=0,
                        help='starting epoch')
    ...
    parser.add_argument('--mixed_precision', type=str, default='False',
                        help='if uses multi-gpu then mixed precision while computing gradients')
...
def main(args):
    ...
    # Construct the directory path for saving images
    args.img_save_dir = osp.join(ROOT_PATH, 'imgs/{0}'.format(osp.join(str(args.CONFIG_NAME), 'train', stamp)))

    # Create necessary directories if not using multiple GPUs or a master node is there
    ...
    # print the count of trainable parameters
    print('**** Trainable G_paras: ',params_count(netG))
    print('**** Trainable D_paras: ',params_count(netD)+params_count(netC))

    # get the fixed data from dataloader
    fixed_img, fixed_sent, fixed_words, fixed_z = get_fix_data(train_dl, valid_dl, text_encoder, args)

    # Save ground truth images if not using multiple GPUs or a master GPU node is executing
    if (args.multi_gpus==True) and (get_rank() != 0):
        None
    else:
    ...
    # Initialize GradScaler if using mixed precision training
    ...
    # Load the noramalization parameters from the numpy array zip file
    m1, s1 = load_npz(args.npz_path)

    # Start epoch for starting training
    start_epoch = 1

    # load from checkpoint if specified
    if args.state_epoch!=1:
        start_epoch = args.state_epoch + 1
        # Determine .pth file path and load the models from it
    ...
    # Print arguments if not using multiple GPUs or a master GPu is executing
    if (args.multi_gpus==True) and (get_rank() != 0):
        None
    else:
        pprint.pprint(args)
    ...
    # Load the intervals for evaluation, sampling and saving the models while training
    ...
    for epoch in range(start_epoch, args.max_epoch, 1):
        ...
        # Update the training information if its not multi-GPUs or master GPU is executing
        if (args.multi_gpus==True) and (get_rank() != 0):
            None
        else:
            if epoch%test_interval==0:
            ...
            # Record current epoch end time
            end_t = time.time()
            # Compute the time needed for current epoch
