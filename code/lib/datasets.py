...
# Function to get fixed data for evaluation
def get_fix_data(train_dl, test_dl, text_encoder, args):
    """
    This function is used to get fixed data for training and evaluation.
    It concatenates fixed image, sentence, word, and noise vectors obtained from both the training and test datasets.
    """
...
# Function to get data for one batch
def get_one_batch_data(dataloader, text_encoder, args):
    """
    This function retrieves data for one batch from the data loader, including images, captions,
    CLIP tokens, sentence embeddings, word embeddings, and keys
    """
...
def prepare_data(data, text_encoder, device):
    """
    This function prepares the data obtained from the data loader,
    encoding captions into CLIP tokens and converting data to the appropriate device.
    """
...
# Function to encode tokens using CLIP
def encode_tokens(text_encoder, caption):
    """
    This function uses the CLIP text encoder to encode text tokens into sentence and word embeddings
    """
    # encode text
    with torch.no_grad():
...
def get_imgs(img_path, bbox=None, transform=None, normalize=None):
    """
    This function loads images from file paths, crops them based on bounding box coordinates,
    applies transformations, and normalizes them to be able to processed by model
    """
...
def get_caption(cap_path,clip_info):
    """
    This function reads captions from text files, selects a random caption,
    tokenizes it using CLIP, and returns the caption along with its tokens
    """
    eff_captions = []
...
#############################################################################################################################
# Dataset preparation functions taken from Caltech University CUB Dataset Pre-processing repository and customized according#
#############################################################################################################################

class TextImgDataset(data.Dataset):
    """
    This is a custom dataset class used for loading text and image data. It handles loading filenames,
    bounding boxes, and retrieving data for each item in the dataset.
    """
    ...
    def load_bbox(self):
        """
        Load the bounding box associated with the images
        """
    ...
    def load_filenames(self, data_dir, split):
        """
         Load filenames of data samples
        """
    ...
    def __getitem__(self, index):
        """
        Retrieves data for a specific index, including images, captions, CLIP tokens, and keys
        """
