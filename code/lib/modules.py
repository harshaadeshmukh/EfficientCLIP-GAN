...
def train(dataloader, netG, netD, netC, text_encoder, image_encoder, optimizerG, optimizerD, scaler_G, scaler_D, args):
    """
     Trains the GAN network on the specified dataset provided through dataloader
    """
    # assigning the hyperparameters received from the command line
    ...
    # loop progress display if multi-GPU training is enabled so that the master GPU print the progress bar
    ...
    for step, data in enumerate(dataloader, 0):
        ...
        with torch.cuda.amp.autocast() if args.mixed_precision else dummy_context_mgr() as mpc:
            ...
            # prepare_data for processing in forward pass
            ...
            # predict real images embeddings and feature vectors
            CLIP_real, real_emb = image_encoder(real)
            real_feats = netD(CLIP_real)

            # Predict similarity score for real images
            pred_real, errD_real = predict_loss(netC, real_feats, sent_emb, negtive=False)

            # predict mismatch with respect to text prompt
            ...
            # synthesize fake images and features embeddings
            noise = torch.randn(batch_size, z_dim).to(device)
            fake = netG(noise, sent_emb)
            CLIP_fake, fake_emb = image_encoder(fake)
            fake_feats = netD(CLIP_fake.detach())

            # Predict similarity score for fake images
            _, errD_fake = predict_loss(netC, fake_feats, sent_emb, negtive=True)

        # Calculate Manifold-Aware Gradient Penalty (MAGP) for reguralization to text semantics in GAN
        ...
        # Update discriminator  networks weights
        if args.mixed_precision:
        ...
        with torch.cuda.amp.autocast() if args.mixed_precision else dummy_context_mgr() as mpc:
            # Creating the fake images feature tensor or embedding
            fake_feats = netD(CLIP_fake)

            # Predict the similarity between fake image features and sentence embeddeding
            output = netC(fake_feats, sent_emb)

            # Compute text similarity between fake embeddings and sentence embeddings
            text_img_sim = torch.cosine_similarity(fake_emb, sent_emb).mean()

            # Calculae the loss for generaotor networks
...
def test(dataloader, text_encoder, netG, PTM, device, m1, s1, epoch, max_epoch, times, z_dim, batch_size):
    """
    Evaluates the GAN network on test dataset
    """
...
def save_model(netG, netD, netC, optG, optD, epoch, multi_gpus, step, save_path):
    """
    Save the model to the persistent storage with the information
    """
    # Check for distributed training
    if (multi_gpus == True) and (get_rank() != 0):
        None
    else:
        # Save generator, discriminator, matcher model
        state = {
            # Save generator, discriminator, matcher model
            'model': {
...
def MA_GP_MP(img, sent, out, scaler):
    """
    Calculate the gradient of the input sentences with respect to the image
     img: real image
     sent: sentence embedding
     out: discriminator similarity score
     scaler: mixed precision scaler object
    """
    # Compute gradients of score w.r.t image and sentence embedding
    ...
    # Rescale gradients to offset scaling by scaler
    grads = [grad * inv_scale for grad in grads]

    with torch.cuda.amp.autocast():
        # Concate the gradients in a single tensor
        grad0 = grads[0].view(grads[0].size(0), -1)
        grad1 = grads[1].view(grads[1].size(0), -1)
        grad = torch.cat((grad0, grad1), dim=1)

        # Compute MA-GP penalty
...
def MA_GP_FP32(img, sent, out):
    """
    MA-GP loss calculation in single precision (FP32)
    """
    # Compute gradients of score w.r.t image and sentence embedding
    grads = torch.autograd.grad(
        outputs=out,
...
def sample(dataloader, netG, text_encoder, save_dir, device, multi_gpus, z_dim, stamp):
    ...
    for step, data in enumerate(dataloader, 0):
        ...
        with torch.no_grad():
            ...
            # Save generated images appended with GPU id
            if multi_gpus == True:
...
def calculate_FID_CLIP_sim(dataloader, text_encoder, netG, CLIP, device, m1, s1, epoch, max_epoch, times, z_dim,
                           batch_size):
    """
      Calculates the FID (Fréchet Inception Distance) and CLIP Similarity
    """
    # initialize clip_cost
    clip_cos = torch.FloatTensor([0.0]).to(device)

    # Setup InceptionV3 model for FID
    dims = 2048
    ...
    # tracking number of images in the dataloader
    n_gpu = dist.get_world_size()
    dl_length = dataloader.__len__()
    imgs_num = dl_length * n_gpu * batch_size * times

    # Initializes array to store InceptionV3 features
    ...
    for time in range(times):
        for i, data in enumerate(dataloader):
            ...
            # Generate fake images
            batch_size = sent_emb.size(0)
            # set generator network to evaluation
            netG.eval()
            with torch.no_grad():
                noise = torch.randn(batch_size, z_dim).to(device)
                fake_imgs = netG(noise, sent_emb, eval=True).float()
                ...
                # Normalize and get InceptionV3 features
                fake = norm(fake_imgs)
                pred = model(fake)[0]
                ...
                # concat pred from multiple GPUs
...
def calc_clip_sim(clip, fake, caps_clip, device):
    ...
    """
    # transform the fake images to CLIP input features
    fake = transf_to_CLIP_input(fake)

    # create the CLIP image features for fake inputs
    fake_features = clip.encode_image(fake)

    # create the CLIP text features for captions
    text_features = clip.encode_text(caps_clip)

    # Calculate cosine similarity between fake image and text features
...
def sample_one_batch(noise, sent, netG, multi_gpus, epoch, img_save_dir, writer):
    """
    Generates a batch of image samples from both train and test dataset
    """
    if (multi_gpus == True) and (get_rank() != 0):
        None
    else:
...
def generate_samples(noise, caption, model):
    """
    Generates a sample from a given noise, caption and model
    """
    with torch.no_grad():
...
def predict_loss(predictor, img_feature, text_feature, negtive):
    """
     Predictor model (netC) makes image-text similarity prediction
     img_feature: visual features of image extracted from discriminator
     text_feature: semantic text features from encoder
     negtive: bool indicating whether image/text is mismatched pair

     negtive=True -> use loss to penalize high similarity scores
     negtive=False -> use loss to penalize low similarity scores
     Overall, encourages high similarity score for matched pairs
     And low similarity score for mismatched pairs
    """
    # Forward pass image and text through prediction model
    output = predictor(img_feature, text_feature)

    # Calculate loss based on "hinge loss" formulation
    err = hinge_loss(output, negtive)

    # Return both similarity score prediction and calculated loss
    return output, err
...
def hinge_loss(output, negtive):
    ...
    parameter -
    output: predicted similarity score
    negtive: bool indicating mismatched text-image pair
    """
    if negtive == False:
        # Real pair, penalize if similarity < 1
        err = torch.mean(F.relu(1. - output))
    else:
        # Mismatched pair, penalize if similarity > -1
        err = torch.mean(F.relu(1. + output))
...
def logit_loss(output, negtive):
    """
     Calculates the binary cross-entropy (BCE) loss and returns the logistic probability
    """
    # Define labels
    ...
    # Convert similarity score to probability using sigmoid
    output = nn.Sigmoid()(output)

    if negtive == False:
        # Use BCE loss for matched pairs
...
# Function has been taken from InceptionV3 repository
