...
def choose_model(model):
    ...
    choose model
    """
    model = importlib.import_module(".%s"%(model), "model")
...
def params_count(model):
    """
    Counts the parameters of the given model
    """
...
def str2bool_dict(dict):
    """
    Funtion to convert a string to the boolean value
    """
...
def merge_args_yaml(args):
    """
    merge the command line arguments and arguments in YAML file
    """
...
def read_txt_file(txt_file):
    """
    For reading the contents of the specified text file
    """
    content = []
...
def load_opt_weights(optimizer, weights):
    """
    load the weights of the optimizer
    """
...
def load_models_opt(netG, netD, netC, optim_G, optim_D, path, multi_gpus):
    """
    Load the model from the given checkpoint file and optimizers parameters
    """
    # Loading the model from the given checkpoint file, initially on CPU
    checkpoint = torch.load(path, map_location=torch.device('cpu'))

    # Loading GAN model from the given checkpoint file
    ...
    # Loading the Generators and discriminator optimizers
...
def load_models(netG, netD, netC, path):
    """
    Function to load model from a given checkpoint
    """
    # Loading the model from the given checkpoint file, initially on CPU
    checkpoint = torch.load(path, map_location=torch.device('cpu'))
...
def save_models_opt(netG, netD, netC, optG, optD, epoch, multi_gpus, save_path):
    """
    Function to save the model with optimizers if it is Master GPU node
    """
...
def save_models(netG, netD, netC, epoch, multi_gpus, save_path):
    """
    save the model without the optimizer if it is Master GPU node
    """
    if (multi_gpus==True) and (get_rank() != 0):
        None
    else:
...
def save_checkpoints(netG, netD, netC, optG, optD, scaler_G, scaler_D, epoch, multi_gpus, save_path):
    """
    Save a model as a checkpoint into a pytorch model file with scalers and optimizers
    """
    if (multi_gpus==True) and (get_rank() != 0):
        None
    else:
...
def save_img(img, path):
    """
    Save a image to specified file
    """
...
# ! Function to taken from the CLIP repository
def transf_to_CLIP_input(inputs):
    """
    Transform the given inputs to CLIP campatible inputs
    """
